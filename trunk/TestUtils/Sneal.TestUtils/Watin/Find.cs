...
namespace Sneal.TestUtils.Watin
{
    ...
    public class Find
    {
        ...
        /// <summary>
        /// Find an element by its id, and if not found, by name.
        /// </summary>
        /// <param name="idorName">Element id or name to find.</param>
        /// <returns><see cref="AttributeConstraint" /></returns>
        /// <example>
        /// <code>ie.Link(Find.ByIdOrName("testlinkid")).Url</code>
        /// </example>
        public static BaseConstraint ByIdOrName(string idorName)
        {
            var idConstraint = new AttributeConstraint(idAttribute, new LooseStringCompare(idorName));
            return idConstraint.Or(new AttributeConstraint(nameAttribute, new LooseStringCompare(idorName)));
        }
        ...
        /// </summary>
        /// <param name="id">
        /// Element id to find. For ASP.NET name mangled IDs, prefix the ID with
        /// a tilde '~'.  For example "ctl001_ctrl002_userText" could be found by
        /// specifying: "~userText" without the quotes.
        /// </param>
        /// <returns><see cref="AttributeConstraint" /></returns>
        /// <example>
        /// <code>ie.Link(Find.ById("testlinkid")).Url</code>
        /// <code>ie.Link(Find.ById("~aspNetMangledId")).Url</code>
        /// </example>
        public static AttributeConstraint ById(string id)
        {
            return new AttributeConstraint(idAttribute, new LooseStringCompare(id));
        }
        ...
        {
            return new AttributeConstraint(nameAttribute, new LooseStringCompare(name));
        }
        ...
        {
            return new AttributeConstraint(textAttribute, new LooseStringCompare(text));
        }
